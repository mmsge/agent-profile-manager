...
and a new root is empty until the account signs in. A sessions server over
each root's own transcripts was designed, built and then held back to 1.1;
[the proposal](proposals/2026-09-15-sessions-mcp.md) records the design, and
how its one registration line was written by Claude Code's own CLI rather
than by this tool.
