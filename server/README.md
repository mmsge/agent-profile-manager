...
| `session_summary(session_id)` | The identity columns, the last reply, a tool histogram, the files touched and the subagents |
...
| `search(query, cwd, since, until, branch, roles, regex, limit)` | Pointers into `get_session`, with a snippet |
...
| `search` regular expression | 200 characters, longer patterns are refused |
